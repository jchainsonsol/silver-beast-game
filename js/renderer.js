...
export class Renderer {
  ...
  resize() {
    ...
    // Always fill the available game viewport.
    // This removes the black bars/dead space on phone, tablet, and PSG1.
    this.scale = Math.max(
  ...
  beginWorldFrame(game) {
    ...
    let desiredX = game?.player?.x ?? 180;
    ...
    const nearestEnemy = game?.enemies
      ...
      ?.sort(
        (a, b) =>
          ...
          Math.abs(b.x - desiredX)
      )[0];

    // Keep the player as the camera target.
    // A close enemy only nudges the view a little toward the fight.
    if (
      nearestEnemy &&
      Math.abs(nearestEnemy.x - desiredX) < 230
    ) {
      desiredX =
        desiredX * 0.88 +
        nearestEnemy.x * 0.12;
    ...
    this.cameraX +=
      (this.targetCameraX - this.cameraX) * 0.10;
